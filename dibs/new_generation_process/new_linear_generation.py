...
def make_synthetic_bayes_net(*,
    ...
    for i in range(n_observations):
        ...
        thetas.append(theta)
        key, subk = random.split(key)
        observation = generative_model.sample_obs(key=subk, n_samples = 1, g = sampled_graph, theta = theta, mean_distr = mean_distr)
...
def make_graph(*, key,n_vars, graph_prior_str, edges_per_node=2):
    """
    ...
    key, subk = random.split(key)
    sampled_graph = graph_dist.sample_G(subk)

    return sampled_graph


def make_linear_gaussian_model(*, key, sampled_graph, n_vars=20, 
    ...
    generative_model = LinearGaussian(
        sampled_graph = sampled_graph, obs_noise=obs_noise,
